...
from upskill.evaluate import load_eval_results_from_artifact_root
...
from upskill.models import ExpectedSpec, Skill, TestCase, TestResult
...
def test_build_fast_agent_command_uses_explicit_contract(tmp_path: Path) -> None:
    ...
    command = build_fast_agent_command(
        ...
        prompt_file=tmp_path / "bundle" / "prompt.txt",
...
async def test_local_fast_agent_executor_normalizes_paths_and_preserves_file_context(
    ...
    async def fake_create_subprocess_exec(*args: str, **kwargs: object) -> FakeProcess:
        ...
        card_index = args.index("--card") + 1
        for index in (config_index, results_index, prompt_index, skills_index, card_index):
